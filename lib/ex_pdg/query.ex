defmodule ExPDG.Query do
  ...
  """
  @spec nodes(Graph.t(), keyword()) :: [Node.t()]
  ...
  def data_flows?(%Graph{} = graph, source_id, sink_id) do
  ...
  def controls?(%Graph{} = graph, controller_id, target_id) do
  ...
  def depends?(%Graph{} = graph, id_a, id_b) do
  ...
  def has_dependents?(%Graph{} = graph, node_id) do
  ...
  def passes_through?(%Graph{} = graph, source_id, sink_id, predicate) do
  ...
  def returns?(%Graph{control_flow: control_flow}, node_id) do
